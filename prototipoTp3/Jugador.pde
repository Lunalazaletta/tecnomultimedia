class Jugador {
  ...
  Jugador(PApplet app, FWorld w, float px, float py, float objX, float objY, float a, float bPos, boolean j) {
   ...
   // c5 = new Ficha(mundo, px+adelanto, py*2, objX, objY, jugadorCol, true);
   // c6 = new Ficha(mundo, px, py*3, objX, objY, jugadorCol, false);
    fichas[0] = c1;
    fichas[1] = c2;
    fichas[2] = c3;
    //fichas[0] = c4;
    ...
    if(conTuio){
      if(id1Presente && posicionando){
        if(j1 && xPuntero1< width/2){
            c3.setPos(xPuntero1, yPuntero1); 
            c3.apuntarAngulo(angulo1 + PI/2);
        }
      }
      if(id2Presente && posicionando){
       ...
       if(id3Presente && posicionando){
        if(j1 && xPuntero3< width/2){
            c1.setPos(xPuntero3, yPuntero3); 
            c1.apuntarAngulo(angulo3 + PI/2);
        }
       }
      if(id4Presente && posicionando){
        if(!j1 && xPuntero4> width/2){
            c3.setPos(xPuntero4, yPuntero4);
            c3.apuntarAngulo(angulo4 + PI/2);
        }
      }
    if(id5Presente && posicionando){
        if(!j1 && xPuntero5> width/2){
            c2.setPos(xPuntero5, yPuntero5); 
            c2.apuntarAngulo(angulo5 + PI/2);
        }
    }
